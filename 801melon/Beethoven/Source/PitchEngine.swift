...
public protocol PitchEngineDelegate: class {
  func pitchEngine(_ pitchEngine: PitchEngine, didReceivePitch pitch: Pitch,withfrequency: Float)
...
//            print("基频%f",frequency)
          ...
          DispatchQueue.main.async { [weak self] in
            guard let `self` = self else { return }
            self.delegate?.pitchEngine(self, didReceivePitch: pitch, withfrequency: frequency )
          }
        } catch {
          DispatchQueue.main.async { [weak self] in
            guard let `self` = self else { return }
